"""Classification: the current model must match or beat the recorded v1.1 result."""
import unittest
...
class TestClassification(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        """Setup dataset and the model, then gather predictions."""
        seed_everything(42)
    ...
    def test_accuracy(self):
        """Train prediction should be perfect; test accuracy should be at least v1.1's."""
        train_acc = accuracy_score(self.y_train, self.y_train_pred)
        test_acc = accuracy_score(self.y_test, self.y_test_pred)
        print(f"\n[cls] test acc {test_acc:.4f} vs v1.1 {V1_CLS_ACC:.4f} (delta {test_acc - V1_CLS_ACC:+.4f})")
        ...
        self.assertGreaterEqual(
            ...
            msg=f"accuracy {test_acc:.4f} regressed >{TOLERANCE} below v1.1 {V1_CLS_ACC}",
        )
